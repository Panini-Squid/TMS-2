...
public class Customer extends User {
    ...
    public void buyCommodityCommand(ArrayList<String> commands) {//购买商品
        // 店铺编号不合法
        if (!Tool.shopIDValid(commands.get(1))) {
            System.out.println("Illegal shop id");
            return;
        ...
        int buyAmount = Integer.parseInt(commands.get(3));
        Commodity c1 = s1.commodityList.get(cID);
        // 购买数量大于商品数量
        if (buyAmount > c1.getCommodityAmount()) {
            System.out.println("Illegal buy quantity");
            return;
        }
        c1.setCommodityAmount(c1.getCommodityAmount() - buyAmount);
